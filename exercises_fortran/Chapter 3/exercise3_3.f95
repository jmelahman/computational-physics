PROGRAM EXERCISE3_3
!   Uses Green's function to solve Poisson's
!   equation with rho=exp(-r)/(8pi)
IMPLICIT NONE
    REAL :: PHI
    REAL :: R
    REAL :: PART1
    ...
    DO I=1,IMAX+1
        R = I*H
        EXACT = 1.0-(R+2)*EXP(-R)/2.0
        CALL INTG1(R,L,PART1)
        CALL INTG2(R,L,PART2)
        PHI = PART1+PART2
...
SUBROUTINE INTG1(R,L,PART1)
!   Uses Green's function and Gaussian quad
!   to solve the first part of equation 3.18
IMPLICIT NONE
...
!dR = dRD/2R
    DO I=1,64
        RD = (XI(I)+1.0)/2.0/R
        VAL = PHIHI(1./RD)*SOURCE(1./RD)/(RD*RD*R*2.)
